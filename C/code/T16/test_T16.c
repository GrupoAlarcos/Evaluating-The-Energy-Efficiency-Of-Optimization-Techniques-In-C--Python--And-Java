...
extern int t2(unsigned int, unsigned int);

int test_identical_output_check_all() {
    unsigned int dividend = 100000000;
    unsigned int exponent = 10;
    unsigned int divisor = 1u << exponent;

    int r1 = t1(dividend, divisor);
    int r2 = t2(dividend, exponent);
...
int test_files_identical() {
    int ok1 = files_identical_ignore_opt(
        "Test1O0/16_division-by-a-power-of-two-denominator-test1.c",
        "Test1O3/16_division-by-a-power-of-two-denominator-test1.c"
    );

    int ok2 = files_identical_ignore_opt(
        "Test2O0/16_division-by-a-power-of-two-denominator-test2.c",
        "Test2O3/16_division-by-a-power-of-two-denominator-test2.c"
    );
    return ok1 && ok2;
}
