...
int test_identical_output_check_all() {
    ...
    ia1(a1, N);
    ia2(a2, N);
    int r1 = t1(a1, N, ierror);
    int r2 = t2(a2, N, ierror);
    ...
    return (memcmp(a1, a2, N * sizeof(int))) == 0 && (r1 == r2);
}
...
int test_files_identical() {
    int ok1 = files_identical_ignore_opt(
        "Test1O0/28_Fission-test1.c",
        "Test1O3/28_Fission-test1.c"
    );

    int ok2 = files_identical_ignore_opt(
        "Test2O0/28_Fission-test2.c",
        "Test2O3/28_Fission-test2.c"
    );
    return ok1 && ok2;
}
