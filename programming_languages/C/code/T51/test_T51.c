...
int test_identical_output_check_all()
{
    ...
    int a1[N];
    int a2[N];
    memcpy(a1, master, sizeof(master));
    memcpy(a2, master, sizeof(master));
    t1(a1, N);
    t2(a2, N, THRESHOLD);
...
int test_files_identical()
{
    int ok1 = files_identical_ignore_opt(
        "Test1O0/51_coarsening-recursion-test1.c",
        "Test1O3/51_coarsening-recursion-test1.c");

    int ok2 = files_identical_ignore_opt(
        "Test2O0/51_coarsening-recursion-test2.c",
        "Test2O3/51_coarsening-recursion-test2.c");
    return ok1 && ok2;
}
