...
def test_identical_output_check_all():
    ...
    minimum = min(val_a, val_b)
    maximum = max(val_a, val_b)
    i = random.randint(minimum, maximum)

    result1 = file1.test(i, maximum, minimum)
    result2 = file2.test(i, maximum, minimum)

    for name, r in [("file1", result1), ("file2", result2)]:
        assert r is not None, f"{name}.test returned None!"
        assert isinstance(r, bool), f"{name}.test should return a boolean, got {type(r)}"
